	       ...
	       uint16_t operation)
{
    ...
    mithril_spi_init(spi_handle, &spi_init_struct);
	return 0;
}
...
static void spi_mithril_xfer(const struct device *dev)
{
    ...
    while (spi_context_tx_on(ctx) || spi_context_rx_on(ctx)) {
        ...
        if(spi_context_tx_on(ctx) && spi_context_rx_on(ctx)) {
            txe=true;
            rxe=true;
            read_data = mithril_spi_xfer(spi_handle, *ctx->tx_buf);
            *ctx->rx_buf = read_data;
        } else if(spi_context_tx_on(ctx)){
            txe=true;
            mithril_spi_transmit(spi_handle, *ctx->tx_buf);
        } else {
            rxe=true;
            mithril_spi_receive(spi_handle,ctx->rx_buf);
        }
